<x-app-layout>
    <x-slot name="header">
            Seguridad - Grupos - {{ $role->name }} - Permisos - Asignar Permisos
    </x-slot>
            ...
            <div class="flex justify-center mt-8">
                <div class="w-full md:w-1/2">
                    <div class="bg-gray-50 shadow-md rounded px-8 pt-6 pb-8 mb-4">
                        <div class="mb-4">
                        {!! Form::open(['route' => ['role.permission.store', ['role' => $role]]]) !!}
                            <div class="text-gray-500">
                                <div class="form-group">
                                    {!! Form::label('menu_id', 'Formulario') !!}
                                    {!! Form::select('menu_id', $menus->pluck('text', 'id'), null, ['class' => 'mt-1 border px-2 py-2 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 shadow-sm']) !!}
                                </div> 
                                <div class="form-group">
                                    {!! Form::label('permission_id', 'Permiso') !!}
                                    {!! Form::select('permission_id', $permissions->pluck('name', 'id'), null, ['class' => 'mt-1 border px-2 py-2 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 shadow-sm']) !!}
                                </div> 
                            </div>
                            @if ($errors->any())
