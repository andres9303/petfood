<x-app-layout>
    <x-slot name="header">
        Compras - Ordenes de compra directas - #{{ $direct_purchase->code ? $direct_purchase->code.'-' : '' }}{{ $direct_purchase->num }} - Editar Compra
    </x-slot>
            ...
            <div class="flex justify-center">
                <div class="w-full">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                        ...
                        {!! Form::model($direct_purchase, ['route' => ['direct-purchase.update', ['direct_purchase' => $direct_purchase]], 'method' => 'put' ]) !!}
                            @include('shopping.direct-purchase.components.form-elements')
                            @if ($errors->any())
                        ...
                        <div class="bg-gray-50 text-gray-500 shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full md:col-span-2 block">
                            @livewire('forms.purchase-form', ['doc' => $direct_purchase])
                        </div>
                    </div>
                </div>
            </div>
